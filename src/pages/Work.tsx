...
const WorkPage = () => {
  ...
  return (
    <>
      <Seo
        title="Work"
        description="Case studies from ZenWebStudio — SaaS platforms, custom web applications, and mobile apps. Problem, solution, stack, and outcome."
        path="/work"
      />
      <PageHero
        eyebrow="Work"
        title={
          <>
            A filterable record of <em className="hl-ember not-italic">what we have shipped.</em>
          </>
        }
        body="Real products from the studio. We describe the problem, the system we built, the stack, and an outcome we can stand behind — no vanity founder counts."
      />
          ...
          {items.map((item) => (
            <article key={item.id} className="glass overflow-hidden rounded-[28px]">
              <div className="relative aspect-[16/10] overflow-hidden">
                <img src={item.cover} alt={`${item.title} cover`} className="h-full w-full object-cover" />
